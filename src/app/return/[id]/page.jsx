...
export default function ReturnPage() {
  ...
  // The orders list page links here as
  // `/return/${order.order_number}?type=return&item=${item.id}`
  // — pull that item id out so we know exactly which line item
  ...
  const submitReturn = async () => {
    // Guard: without an item id the backend can't attach this request
    // to a specific line item (required since the create() endpoint
    // validates order_item_id as required|exists:order_items,id).
    if (!orderItemId) {
      setError("Missing item reference — please go back and try again from your orders page.");
      return;
    }
    ...
    try {
      ...
      // customer_id is intentionally NOT sent — the backend derives it
      // from the authenticated session (Auth::guard('customer')->id()),
      // so a client can never create a return for someone else's order.
      await API.post("/returns/create", {
        order_id: params.id, // order_number
            ...
            {!isTrackPage ? (
              <>
                <div className="max-w-2xl">
                  ...
                  {/* Item reference (read-only confirmation, not editable) */}
                  {orderItemId && (
                    <div className="mb-4">
                      <input
                        type="text"
