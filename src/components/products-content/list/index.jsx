  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      button.style.display =
        footer.getBoundingClientRect().top < window.innerHeight ? "none" : "flex";
    };
  ...
  return (
    <>
      <style>{`
        ...
        .product-count {
          font-family: "Century Gothic", Futura, "Trebuchet MS", sans-serif;
          font-size: 11px; letter-spacing: 0.1em; color: rgba(28,24,20,0.45);
        }
      `}</style>
      ...
      <div className="bg-white min-h-screen">
        ...
        {/* ── TOP BAR ────────────────────────────────────────── */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.2rem clamp(1rem, 4vw, 3rem)", borderBottom: "1px solid rgba(196,168,130,0.15)", flexWrap: "wrap", gap: "1rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexWrap: "wrap" }}>
            ...
            {[...filters.size, ...filters.color].map((item) => (
              <span key={item} className="active-tag">
                {item}
                <button
                  onClick={() => setFilters({ ...filters, size: filters.size.filter((s) => s !== item), color: filters.color.filter((c) => c !== item) })}
                  style={{ background: "none", border: "none", cursor: "pointer", color: "#1C1814", padding: 0, display: "flex" }}
                >
                  <CloseIcon />
                </button>
              </span>
          ...
          <div style={{ position: "relative" }}>
            <button className="sort-btn" onClick={() => setOpenSort(!openSort)}>
            ...
            <AnimatePresence>
              {openSort && (
                <motion.div
                  ...
                  style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", background: "#FAFAF7", border: "1px solid rgba(196,168,130,0.3)", minWidth: 200, zIndex: 100 }}
                >
                  {Object.entries(sortLabels).map(([key, label]) => (
                    <div key={key} className={`sort-option ${sort === key ? "active" : ""}`} onClick={() => { setSort(key); setOpenSort(false); }}>
                      {label}
                    </div>
                  ))}
                </motion.div>
        ...
        {/* ── FLOAT BUTTON ───────────────────────────────────── */}
        <div id="filterBtn" style={{ position: "fixed", bottom: "2rem", left: "50%", transform: "translateX(-50%)", zIndex: 50 }}>
          <motion.button whileTap={{ scale: 0.96 }} className="filter-float-btn" onClick={() => setShowFilters(true)}>
            <FilterIcon />
            Filter & Sort
            {activeCount > 0 && (
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 18, height: 18, borderRadius: "50%", background: "#C4A882", color: "#1C1814", fontSize: 9, fontWeight: 600, marginLeft: 2 }}>
                {activeCount}
              </span>
            )}
          </motion.button>
          ...
          {showFilters && (
            <>
              <motion.div
                ...
                style={{ position: "fixed", inset: 0, zIndex: 40, background: isDesktop ? "rgba(28,24,20,0.25)" : "rgba(28,24,20,0.4)", backdropFilter: "blur(2px)" }}
              ...
              <motion.div
                ...
                onTouchEnd={() => { if (!isDesktop && touchStartY - touchEndY < -80) setShowFilters(false); }}
                style={{
                  position: "fixed",
                  zIndex: 50,
                  background: "#FAFAF7",
                  ...
                  ...(isDesktop
                    ? { top: 0, left: 0, height: "100vh", width: 400, borderRight: "1px solid rgba(196,168,130,0.25)" }
                    : { bottom: 0, left: 0, right: 0, maxHeight: "88vh", borderTop: "1px solid rgba(196,168,130,0.25)" }),
                }}
              >
                {/* Drag handle — mobile only */}
                {!isDesktop && (
                  <div style={{ display: "flex", justifyContent: "center", padding: "1rem 0 0", flexShrink: 0 }}>
                    <div style={{ width: 36, height: 3, borderRadius: 99, background: "rgba(28,24,20,0.2)" }} />
                  </div>
                )}

                {/* Header — fixed height */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.5rem 2rem", borderBottom: "1px solid rgba(196,168,130,0.2)", flexShrink: 0 }}>
                  <div>
                    <h2 className="heading-font" style={{ fontSize: "0.85rem", fontWeight: 400, letterSpacing: "0.25em", textTransform: "uppercase", color: "#1C1814" }}>
                      Refine
                    </h2>
                    {activeCount > 0 && (
                      <p className="font-futura" style={{ fontSize: 11, color: "#B09880", marginTop: 2 }}>
                        {activeCount} filter{activeCount > 1 ? "s" : ""} active
                      </p>
                    )}
                  </div>
                  <button onClick={() => setShowFilters(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#1C1814", padding: "0.25rem" }}>
                    <CloseIcon />
                  </button>
                </div>

                {/* Body — grows, scrolls */}
                <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "2rem", display: "flex", flexDirection: "column", gap: "2.5rem" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      {Object.entries(sortLabels).map(([key, label]) => (
                        <button
                          key={key}
                          onClick={() => setSort(key)}
                          style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.85rem 1rem", border: "1px solid", borderColor: sort === key ? "#1C1814" : "rgba(28,24,20,0.12)", background: sort === key ? "#1C1814" : "transparent", cursor: "pointer", fontFamily: `"Century Gothic", Futura, "Trebuchet MS", sans-serif`, fontSize: 12, letterSpacing: "0.06em", color: sort === key ? "#E8E0D0" : "#1C1814", transition: "all 0.25s" }}
                        >
                          {label}
                          {sort === key && (
                            <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                              <path d="M1.5 5L4 7.5L8.5 2.5" stroke="#C4A882" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                          )}
                        </button>
                      ))}
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
                        {colors.map((color) => {
                          ...
                          return (
                            <div key={color} onClick={() => { const updated = active ? filters.color.filter((c) => c !== color) : [...filters.color, color]; setFilters((prev) => ({ ...prev, color: updated })); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.4rem", cursor: "pointer" }}>
                              <div className={`color-swatch ${active ? "active" : ""}`} style={{ backgroundColor: color }} />
                              <span className="font-futura" style={{ fontSize: 9, letterSpacing: "0.1em", textTransform: "capitalize", color: "#6B5B4E" }}>{color}</span>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "1.5rem" }}>
                      <p className="drawer-section-title" style={{ margin: 0 }}>Max Price</p>
                      <span className="heading-font" style={{ fontSize: "1rem", fontWeight: 300, color: "#1C1814", letterSpacing: "0.05em" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: "0.4rem" }}>
                      <span className="font-futura" style={{ fontSize: 10, color: "rgba(28,24,20,0.35)" }}>₹0</span>
                      <span className="font-futura" style={{ fontSize: 10, color: "rgba(28,24,20,0.35)" }}>₹10,000</span>
                    </div>
                  </div>
                ...
                <div style={{ flexShrink: 0, display: "flex", gap: "0.75rem", padding: "1.25rem 2rem", borderTop: "1px solid rgba(196,168,130,0.2)", background: "#FAFAF7" }}>
                  <button
                    onClick={() => setFilters({ size: [], color: [], maxPrice: 10000 })}
                    style={{ flex: 1, padding: "0.9rem", border: "1px solid rgba(28,24,20,0.2)", background: "transparent", fontFamily: josefin.style.fontFamily, fontSize: 10, letterSpacing: "0.25em", textTransform: "uppercase", color: "#1C1814", cursor: "pointer" }}
                  >
                    Clear All
                  </button>
                  <button
                    onClick={() => setShowFilters(false)}
                    style={{ flex: 2, padding: "0.9rem", border: "none", background: "#1C1814", fontFamily: josefin.style.fontFamily, fontSize: 10, letterSpacing: "0.25em", textTransform: "uppercase", color: "#E8E0D0", cursor: "pointer" }}
                  >
